...
async def update_rule(task_type: str, req: RouteRuleRequest):
    ...
    if cursor.rowcount == 0:
        raise HTTPException(status_code=404, detail=f"No rule for task_type '{task_type}'")
    return {"success": True}
...
async def test_model(provider_id: str, model_name: str):
    ...
    if not base_url:
        return {"success": False, "error": f"Provider '{provider_id}' has no base URL configured"}
    try:
        ...
        return {"success": False, "error": f"Cannot connect to provider '{provider_id}' at {base_url}"}
